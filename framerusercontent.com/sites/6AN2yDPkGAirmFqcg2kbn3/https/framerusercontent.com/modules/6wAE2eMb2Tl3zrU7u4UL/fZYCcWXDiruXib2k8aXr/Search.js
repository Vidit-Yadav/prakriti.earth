...
import {
  ...
  SearchLayoutType,
  SearchIconType,
} from "https://framerusercontent.com/modules/tV9haTHllpHHc9Fjue2H/JehrJSLhXvzeRdsqPjt4/SearchModal.js";
...
var EntryPointOptions; // SITE SEARCH
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event) => {
      ...
      const isWithinSearchHeader = Boolean(
        event.target instanceof Element && event.target.closest("[role=search]")
      );
 ...
 */ const EntryPoint = withCSS(
  function EntryPoint(props) {
    ...
    const baseInputFontSize = props.inputOptions?.inputFont?.fontSize
      ...
      : "16px"; // The font size is set to a minimum of `16px` on Safari with touch screens
    ...
    const theme = {
      ...
      titleFont:
        ...
        !isEmptyObject(props.resultOptions.titleFont)
          ? props.resultOptions.titleFont
          : { fontSize: 14, fontFamily: DEFAULT_FONT_FAMILY, fontWeight: 500 },
      subtitleFont:
        props.resultOptions.subtitleOptions?.subtitleFont &&
        !isEmptyObject(props.resultOptions.subtitleOptions.subtitleFont)
          ? props.resultOptions.subtitleOptions.subtitleFont
          : { fontSize: 12, fontFamily: DEFAULT_FONT_FAMILY, fontWeight: 500 },
      inputFont:
        props.inputOptions?.inputFont &&
        !isEmptyObject(props.inputOptions.inputFont)
          ? props.inputOptions.inputFont
          : { fontSize: 16, fontFamily: DEFAULT_FONT_FAMILY, fontWeight: 500 }, // Keep separate so we can more easily override
...
addPropertyControls(EntryPoint, {
  urlScope: {
    title: "Scope", // @ts-ignore - Internal
  ...
  modalOptions: {
    ...
    controls: {
      ...
      shadow: {
        ...
        controls: {
          color: { type: ControlType.Color, defaultValue: "rgba(0,0,0,0.2)" },
      ...
      [animationKeyFromLayout(SearchLayoutType.FixedTop)]: {
        ...
        controls: {
          ...
          y: { type: ControlType.Number, defaultValue: 0, min: -100, max: 100 },
          transition: { type: ControlType.Transition },
        },
      },
      [animationKeyFromLayout(SearchLayoutType.Sidebar)]: {
        ...
        controls: {
          ...
          x: { type: ControlType.Number, defaultValue: 0, min: -1e3, max: 1e3 },
          transition: { type: ControlType.Transition },
        },
      },
    },
  ...
  resultOptions: {
    ...
    controls: {
      ...
      titleFont: {
        title: "Title", // @ts-ignore - Internal
        type: ControlType.Font,
...
export const __FramerMetadata__ = {
  exports: {
    Shadow: { type: "tsType", annotations: { framerContractVersion: "1" } },
    EntryPointProps: {
      type: "tsType",
