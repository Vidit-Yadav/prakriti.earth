...
const defaultVideo =
  "https://framerusercontent.com/assets/MLWPbW1dUQawJLhhun3dBwpgJak.mp4"; // Reduce renders
...
function useAutoplayBehavior({
  ...
  const behavesAsGif = // passing `playing === true` on mount indicates that the video should
    ...
    loop &&
    playsinline &&
    !controls && // Some users of the <Video> component use it by wrapping it with
...
const VideoMemo = /*#__PURE__*/ memo(function VideoInner(props) {
  ...
  const autoplayBehavior = isStaticRenderer
    ? "no-autoplay"
    : useAutoplayBehavior({ playingProp, muted, loop, playsinline, controls });
  const isInViewport = isStaticRenderer ? true : useInView(videoRef);
  const isCloseToViewport = isStaticRenderer
    ? false
    : useInView(videoRef, { margin: "10%", once: true }); // Video elements behave oddly at 100% duration
  const startTime = startTimeProp === 100 ? 99.9 : startTimeProp;
  const { play, pause, setProgress, isPlaying } = usePlaybackControls(videoRef); // Pause/play via props
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      return; // forces a poster to show up when the video is in an iframe; this is needed when `poster` changes
...
export const __FramerMetadata__ = {
  exports: {
    VideoProps: { type: "tsType", annotations: { framerContractVersion: "1" } },
    titleCase: {
      type: "function",
