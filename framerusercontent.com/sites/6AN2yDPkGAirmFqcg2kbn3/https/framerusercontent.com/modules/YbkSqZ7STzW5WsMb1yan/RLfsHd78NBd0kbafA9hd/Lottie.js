 ...
 */ export function Lottie(props) {
  ...
  const animationInfo = useRef({ data: null, animation: null, duration: null });
  ...
  const scrubTo = useCallback((normalizedProgress, shouldPlayAfter) => {
    if (!animationInfo.current.duration || !animationInfo.current.animation)
      return; // Lottie gets weird when scrubbing to the exact last frame
  ...
  ); // Initial setup or re-init when source changes
  useEffect(() => {
    let cleanup;
