...
import { SearchResultTitleType } from "https://framerusercontent.com/modules/tV9haTHllpHHc9Fjue2H/JehrJSLhXvzeRdsqPjt4/SearchModal.js";
import {
  getCachedIndex,
  setCachedIndex,
  isDefaultLocaleId,
} from "https://framerusercontent.com/modules/uU1mtMKXsrVAg8N5hW7w/wltkapIGIk88vadN6BJZ/cachedIndex.js";
 ...
 */ function getNormalizedString(text) {
  ...
  return text
    .normalize("NFD") // From: https://stackoverflow.com/a/37511463
...
export function useSearch(query, settings) {
  ...
  useEffect(() => {
    async function loadSearchIndex() {
      ...
      const response = await fetch(searchIndexURL); // If the file doesn't exist yet, it might be still generating.
      if (response.status === 403 || response.status === 404) {
        log("Index not found"); // If there is no cached index, show the pending index message.
        // Otherwise use the cache to as the index to search.
        if (cacheResult.status === "miss") {
          setStatus("pending-index-generation");
          log("No cache to use");
