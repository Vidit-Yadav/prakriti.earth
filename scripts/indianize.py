...
# Define ordered replacement tuples: (pattern/exact, replacement)
# Specific phrases first, followed by general brand tokens

PHRASE_REPLACEMENTS = [
    # Full titles & headings
    ...
    # Geography & Locations (replace US/Chile foreign address with Indian context)
...
def replace_in_file(filepath):
    ...
    # 2. General brand replacements (careful word boundaries where appropriate)
    # Replace 'Alethia' -> 'Prakriti'
    content = content.replace('Alethia', 'Prakriti')
    content = content.replace('ALETHIA', 'PRAKRITI')
    content = content.replace('alethia.earth', 'prakriti.earth')

    # For lowercase 'alethia', replace when standalone or in identifiers
    # like "data-framer-name=\"alethia\"" or "alethia"
    content = re.sub(r'\balethia\b', 'prakriti', content)

    if content != orig_content:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        # Count remaining alethia occurrences
        remaining = len(re.findall(r'alethia', content, re.IGNORECASE))
        return 1, f"Updated (remaining: {remaining})"
    else:
        return 0, "Unchanged"

def main():
    target_exts = {'.html', '.mjs', '.js', '.json', '.css'}
    ...
    print(f"\nScan complete: {scanned} files scanned, {total_modified} files modified.")
